...
calendar, class, concert, conference, date, dates, event, events, google maps, meeting, modern tribe, Organizer, seminar, summit, tribe, venue, widget, workshop


Microformat support

when configuring venues, organizers, speakers, and sponsors, set a featured if you want such an image displayed on the single event page.

Widgets

	Upcoming Events widget. 
	
	WooEvents Pro includes a mini calendar widget! See http://ignitewoo.com for plugin details.
	
custom templates

	Copy the templates subdirectory to your theme directory, renamed as "ignitewoo-events"
	
	Example, copy to 
		/wp-content/themes/twentytwelve/ignitewoo-events
		
	OR 
	
	create a new template file in your theme called single-ignitewoo_event.php. A good starting point is to copy your theme's single post template, then somewhere in the loop include the plugin's ignitewoo-event-details-template.php file, or copy the code from that file into your new template. 
	
Shortcodes

	The plugin includes a shortcode "ignitewoo_events_calendar" 
	
	Examples: 
	
		[ignitewoo_events_calendar]
		
			Displays regular events and ticket events on the calendar
		
		[ignitewoo_events_calendar type="events"]
		
			Display only non-ticketing events
		
		[ignitewoo_events_calendar type="tickets"]
		
			Display only ticket events - *** REQUIRES THE WOOEVENTS PRO PLUGIN ***
...
It's 100% architected around WordPress custom post types and taxonomies, and fully supports custom post type templates. No strange PHP code libraries and no proprietary data storage. If you're familiar with custom post types and post meta then then you'll off to the races in no time flat. 
...
Plus, WooEvents Pro comes with Google Calendar and iCal suppport; a mini-calendar widget, ticket purchase limits for anti-scalper protection; printable tickets; QR code support for rapid check in at the door; the ability to put tickets on sale; custom form generator for data collection, upsells and presales; *and* you gain the ability to use a wide range of other powerful WooCommerce add-ons. For example, use our Scheduled Sales plugin to start and stop ticket sales at any date and time; use our Loyalty Rewards plugin to create a reward points program for frequent ticket buyers; use our Wholesale Pricing plugins to offer discounts to bulk buyers; use our Gift Certificates plugin so people can gift tickets to other people; *and the list goes on.*
